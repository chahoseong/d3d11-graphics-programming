...
float DiffuseToonShading(float value)
{
    if (value <= 0)
    {
        return 0.4;
...
float SpecularToonShading(float value)
{
    if (value <= 0.1)
    {
        return 0.4;
                            ...
                            out float4 spec)
{
    ...
    if (diffuseFactor > 0.0f)
    {
        ...
        diffuse = DiffuseToonShading(diffuseFactor) * mat.Diffuse * L.Diffuse;
        spec = SpecularToonShading(specFactor) * mat.Specular * L.Specular;
    }
}
                       ...
                       out float4 spec)
{
    ...
    if (diffuseFactor > 0.0)
    {
        ...
        diffuse = DiffuseToonShading(diffuseFactor) * mat.Diffuse * L.Diffuse;
        spec = DiffuseToonShading(specFactor) * mat.Specular * L.Specular;
    }
