...
class TreeSpriteGraphicsPipeline : public GraphicsPipeline
{
	struct alignas(16) FrameData
	{
		std::array<DirectionalLight, 3> dirLights;
...
public:
	TreeSpriteGraphicsPipeline(ID3D11Device* device);
	...
	void SetDirLights(const DirectionalLight* lights, int count);
	void SetMaterial(const Material& mat);
	void SetTreeTextureMapArray(Microsoft::WRL::ComPtr<ID3D11ShaderResourceView> SRV);
	void SetUseTexture(bool enabled);
	void SetFogEnabled(bool enabled);
	void SetAlphaClip(bool enabled);
...
private:
	...
	Microsoft::WRL::ComPtr<ID3D11ShaderResourceView> mTreeTextureMapArray;
};
...
public:
	...
	static TreeSpriteGraphicsPipeline* TreeSprite;
};
