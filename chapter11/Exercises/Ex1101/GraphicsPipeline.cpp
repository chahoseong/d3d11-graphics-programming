...
Microsoft::WRL::ComPtr<ID3DBlob> GraphicsPipeline::CompileShader(const std::wstring& filename, const std::string& target) const
{
	...
	{
		if (errorMsg) {
			std::cerr << static_cast<const char*>(errorMsg->GetBufferPointer()) << std::endl;
		}
		return nullptr;
	}
...
CylinderGraphicsPipeline::CylinderGraphicsPipeline(ID3D11Device* device)
	: GraphicsPipeline(device), mFrameCB(device), mObjectCB(device)
{
	mVertexShader = CreateVertexShader(L"cylinder.vertex.hlsl", &mCompiledVertexShader);
	mGeometryShader = CreateGeometryShader(L"cylinder.geo.hlsl", nullptr);
	mPixelShader = CreatePixelShader(L"cylinder.pixel.hlsl", nullptr);
}

void CylinderGraphicsPipeline::SetWorld(DirectX::CXMMATRIX M)
{
	XMStoreFloat4x4(&mObjectCB.data.world, XMMatrixTranspose(M));
}

void CylinderGraphicsPipeline::SetWorldInvTranspose(DirectX::CXMMATRIX M)
{
	XMStoreFloat4x4(&mObjectCB.data.worldInvTranspose, XMMatrixTranspose(M));
}

void CylinderGraphicsPipeline::SetWorldViewProj(DirectX::CXMMATRIX M)
{
	XMStoreFloat4x4(&mObjectCB.data.worldViewProj, XMMatrixTranspose(M));
}

void CylinderGraphicsPipeline::SetEyePosW(const DirectX::XMFLOAT3& v)
{
	mFrameCB.data.eyePosW = v;
}

void CylinderGraphicsPipeline::SetFogColor(const DirectX::FXMVECTOR v)
{
	XMStoreFloat4(&mFrameCB.data.fogColor, v);
}

void CylinderGraphicsPipeline::SetFogStart(float f)
{
	mFrameCB.data.fogStart = f;
}

void CylinderGraphicsPipeline::SetFogRange(float f)
{
	mFrameCB.data.fogRange = f;
}

void CylinderGraphicsPipeline::SetDirLights(const DirectionalLight* lights, int count)
{
	for (int i = 0; i < count; ++i) {
		mFrameCB.data.dirLights[i] = lights[i];
	}
}

void CylinderGraphicsPipeline::SetMaterial(const Material& mat)
{
	mObjectCB.data.material = mat;
}

void CylinderGraphicsPipeline::Apply(ID3D11DeviceContext* context)
{
	mFrameCB.Update(context);
	mObjectCB.Update(context);
	...
	context->GSSetConstantBuffers(1, 1, mObjectCB.handle.GetAddressOf());

	context->PSSetShader(mPixelShader.Get(), nullptr, 0);
	ID3D11Buffer* cbuffers[] = { mFrameCB.handle.Get(), mObjectCB.handle.Get() };
	context->PSSetConstantBuffers(0, 2, cbuffers);
}

ShaderBytecodeView CylinderGraphicsPipeline::GetVertexShaderBytecode() const
{
	return { mCompiledVertexShader->GetBufferPointer(), mCompiledVertexShader->GetBufferSize() };
}
...
CylinderGraphicsPipeline* Pipelines::Cylinder = nullptr;

void Pipelines::InitAll(ID3D11Device* device)
{
	Basic = new BasicGraphicsPipeline(device);
	Cylinder = new CylinderGraphicsPipeline(device);
}
