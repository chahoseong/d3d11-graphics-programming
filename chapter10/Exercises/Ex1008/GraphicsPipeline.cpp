...
// ColorGraphicsPipeline
ColorGraphicsPipeline::ColorGraphicsPipeline(ID3D11Device* device)
	: GraphicsPipeline(device), mFrameCB(device)
{
	mVertexShader = CreateVertexShader(L"color.vertex.hlsl", &mCompiledVertexShader);
	mPixelShader = CreatePixelShader(L"color.pixel.hlsl", nullptr);
}

void ColorGraphicsPipeline::SetColor(const DirectX::FXMVECTOR color)
{
	XMStoreFloat4(&mFrameCB.data.Color, color);
}

void ColorGraphicsPipeline::SetProjection(DirectX::CXMMATRIX M)
{
	XMStoreFloat4x4(&mFrameCB.data.Projection, XMMatrixTranspose(M));
}

void ColorGraphicsPipeline::Apply(ID3D11DeviceContext* context) const
{
	mFrameCB.Update(context);

	context->VSSetShader(mVertexShader.Get(), nullptr, 0);
	context->VSSetConstantBuffers(0, 1, mFrameCB.handle.GetAddressOf());

	context->PSSetShader(mPixelShader.Get(), nullptr, 0);
	context->PSSetConstantBuffers(0, 1, mFrameCB.handle.GetAddressOf());
}

ShaderBytecodeView ColorGraphicsPipeline::GetVertexShaderBytecode() const
{
	return { mCompiledVertexShader->GetBufferPointer(), mCompiledVertexShader->GetBufferSize() };
}
...
ColorGraphicsPipeline* Pipelines::Color = nullptr;

void Pipelines::InitAll(ID3D11Device* device)
{
	Basic = new BasicGraphicsPipeline(device);
	Color = new ColorGraphicsPipeline(device);
}
