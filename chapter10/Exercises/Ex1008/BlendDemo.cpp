...
bool BlendApp::Init()
{
	...
	rectDesc.Usage = D3D11_USAGE_IMMUTABLE;
	rectDesc.ByteWidth = sizeof(Vertex::Pos) * 6;
	rectDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	rectDesc.CPUAccessFlags = 0;
	rectDesc.MiscFlags = 0;
	rectDesc.StructureByteStride = 0;
	D3D11_SUBRESOURCE_DATA vinitData;
	vinitData.pSysMem = rect;
	ThrowIfFailed(md3dDevice->CreateBuffer(&rectDesc, &vinitData, mRectVB.GetAddressOf()));
...
void BlendApp::DrawScene()
{
	...
	md3dImmediateContext->OMSetDepthStencilState(RenderStates::OverdrawCalcDSS.Get(), 1);
	md3dImmediateContext->OMSetBlendState(RenderStates::NoRenderTargetWriteBS.Get(), blendFactor, 0xffffffff);
	...
	offset = 0;
	md3dImmediateContext->IASetVertexBuffers(0, 1, mRectVB.GetAddressOf(), &stride, &offset);
	XMVECTOR colors[] = { Colors::White, Colors::Green, Colors::Yellow, Colors::Magenta, Colors::Red, Colors::Black };
	Pipelines::Color->SetProjection(XMLoadFloat4x4(&mProj));

	for (int i = 0; i < 6; ++i) {
		md3dImmediateContext->OMSetDepthStencilState(RenderStates::OverdrawVisualDSS.Get(), i+1);
		Pipelines::Color->SetColor(colors[i]);
		Pipelines::Color->Apply(md3dImmediateContext.Get());
