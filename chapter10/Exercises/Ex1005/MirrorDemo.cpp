...
private:
	...
	ComPtr<ID3D11DepthStencilState> mWallDSS;
	ComPtr<ID3D11DepthStencilState> mSkullDSS;
...
bool MirrorApp::Init()
{
	...
	ThrowIfFailed(md3dDevice->CreateDepthStencilState(&wallDesc, mWallDSS.GetAddressOf()));

	D3D11_DEPTH_STENCIL_DESC skullDesc;
	// ZeroMemory(&skullDesc, sizeof(D3D11_DEPTH_STENCIL_DESC));
	...
	ThrowIfFailed(md3dDevice->CreateDepthStencilState(&skullDesc, mSkullDSS.GetAddressOf()));
...
void MirrorApp::DrawScene()
{
	...
	md3dImmediateContext->OMSetDepthStencilState(mWallDSS.Get(), 0);
	...
	md3dImmediateContext->OMSetDepthStencilState(mSkullDSS.Get(), 0);
