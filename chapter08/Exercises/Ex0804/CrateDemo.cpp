...
private:
	...
	ComPtr<ID3D11ShaderResourceView> mFlareDiffuseMapSRV;
	ComPtr<ID3D11ShaderResourceView> mFlareAlphaMapSRV;
...
bool CrateApp::Init()
{
	...
	CreateDDSTextureFromFile(
		...
		L"../flare.dds",
		nullptr,
		mFlareDiffuseMapSRV.GetAddressOf()
	);
	...
	CreateDDSTextureFromFile(
		...
		L"../flarealpha.dds",
		nullptr,
		mFlareAlphaMapSRV.GetAddressOf()
	);
...
void CrateApp::UpdateScene(float dt)
{
	...
	// rotate flare texture
	static float rotation = 0;
	rotation += XM_PI * dt;
	XMMATRIX T = XMMatrixTranslation(-0.5f, -0.5f, 0.0f) * XMMatrixRotationZ(rotation) * XMMatrixTranslation(0.5f, 0.5f, 0.0f);
	XMStoreFloat4x4(&mTexTransform, T);
}
...
void CrateApp::DrawScene()
{
	...
	Pipelines::Basic->SetFlareDiffuseMap(mFlareDiffuseMapSRV);
	Pipelines::Basic->SetFlareAlphaMap(mFlareAlphaMapSRV);
