...
bool CrateApp::Init()
{
	...
	Pipelines::Basic->SetSamplerState(RenderStates::PointSS);
...
void CrateApp::BuildGeometryBuffers()
{
	...
	for (size_t i = 0; i < box.Vertices.size(); ++i, ++k) {
		...
		vertices[k].Tex.x = box.Vertices[i].TexC.x / 0.5f;
		vertices[k].Tex.y = box.Vertices[i].TexC.y / 0.5f;
	}
...
void CrateApp::DrawImGui()
{
	...
	static int samplerSelection = 0;
	ImGui::Text("Select sampler state:");

	if (ImGui::RadioButton("Point", &samplerSelection, 0)) {
		Pipelines::Basic->SetSamplerState(RenderStates::PointSS);
	}
	if (ImGui::RadioButton("Linear", &samplerSelection, 1)) {
		Pipelines::Basic->SetSamplerState(RenderStates::LinearSS);
	}
	if (ImGui::RadioButton("Anisotropy", &samplerSelection, 2)) {
		Pipelines::Basic->SetSamplerState(RenderStates::AnisotropicSS);
	}
	if (ImGui::RadioButton("Warp", &samplerSelection, 3)) {
		Pipelines::Basic->SetSamplerState(RenderStates::WarpSS);
	}
	if (ImGui::RadioButton("Clamp", &samplerSelection, 4)) {
		Pipelines::Basic->SetSamplerState(RenderStates::ClampSS);
	}
	if (ImGui::RadioButton("Mirror", &samplerSelection, 5)) {
		Pipelines::Basic->SetSamplerState(RenderStates::MirrorSS);
	}
