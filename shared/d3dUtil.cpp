...
namespace
{
	bool HasDdsExtension(const std::wstring& filename)
	{
		...
		return _wcsicmp(filename.c_str() + filename.size() - 4, L".dds") == 0;
	}
}
		...
		std::vector<std::wstring>& filenames)
{
	...
	// won't be used by the GPU (0 bind flags); they are just used as
	// copy sources to build the array.
	//
	...
	for(UINT i = 0; i < size; ++i)
	{
		...
		if (HasDdsExtension(filenames[i]))
		{
		...
		ThrowIfFailed(resource.As(&srcTex[i]));
	}
	...
	ThrowIfFailed(device->CreateTexture2D( &texArrayDesc, 0, &texArray));
	...
	ThrowIfFailed(device->CreateShaderResourceView(texArray.Get(), &viewDesc, &texArraySRV));
...
ComPtr<ID3D11ShaderResourceView> d3dHelper::CreateRandomTexture1DSRV(ID3D11Device* device)
{
    ...
    ThrowIfFailed(device->CreateShaderResourceView(randomTex, &viewDesc, &randomTexSRV));
...
void ExtractFrustumPlanes(XMFLOAT4 planes[6], CXMMATRIX T)
{
	...
	for(int i = 0; i < 6; ++i)
	{
		DirectX::XMVECTOR v = XMPlaneNormalize(XMLoadFloat4(&planes[i]));
